    ...
    include_once('../config/class/petshop_class.php');
      ...
      $Petshop = new Petshop_class();
        if(ISSET($_POST)){
               ...
               // $notallow_edit = trim($_POST['notallow_edit']);
               // $notallow_delete = trim($_POST['notallow_delete']);

               $Petshop->add_usergroup($user_id, $status, $description, $allow_add, $allow_edit, $allow_delete, $allow_export);

                  echo '<script type="text/javascript">';
                  echo 'setTimeout(function () { swal("Add User Group Successfully!","Message!","success");';
                  echo '}, 1000);</script>';
                  echo '<script type="text/javascript">';
